...
{
    public void CalculateTotals(DraftInvoice draft)
    {
        ...
        draft.TotalNdsAmount = decimal.Round(totalNds, 2, MidpointRounding.AwayFromZero);
        draft.TotalWithNds = decimal.Round(totalWithoutNds + totalNds, 2, MidpointRounding.AwayFromZero);

        draft.UpdatedAt = DateTime.UtcNow;
    }
    
    public DraftInvoiceLine CalculateLine(DraftInvoiceLine line)
    {
        ArgumentNullException.ThrowIfNull(line);

        line.AmountWithoutNds = decimal.Round(line.UnitPrice * line.Quantity, 2, MidpointRounding.AwayFromZero);
